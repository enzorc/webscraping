...
def lecture_exposant(url):
    ...
    try:
        ...
        rep["societe"] = np.nan
    try:
       erhe texte = soup.find('div', "vc_row wpb_row vc_inner vc_row-fluid").find_previous("div").text.strip()
    ...
    try:
        ...
        rep["email"] = np.nan
    try:
    hrthrt    adresse = soup.find("div", attrs = {'id' :'sidebar'}).find('div','element').next_sibling()
        rep["adresse"] = adresse
    except:
        rep["adresse"] = np.nan
    try:
    hrth    infos = soup.find("div", "vc_column-inner vc_custom_1532359577405").text.replace('\n',' ').strip()
